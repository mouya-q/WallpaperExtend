...
Image::Image(int w, int h) : imageData(w, h), valid(true) {}
...
bool Image::loadFromBitmap(JNIEnv* env, jobject bitmap) {
    ...
    imageData.pixels.resize(info.width * info.height);
    void* pixels = nullptr;
    if (AndroidBitmap_lockPixels(env, bitmap, &pixels) < 0) {
        LOGD("Failed to lock pixels");
        return false;
    }
    memcpy(imageData.pixels.data(), pixels, imageData.pixels.size() * sizeof(uint32_t));
...
jobject Image::createBitmap(JNIEnv* env) const {
    ...
    jobject bitmap = env->CallStaticObjectMethod(bitmapClass, createMethod,
            imageData.width, imageData.height, config);
    env->DeleteLocalRef(configClass);
    env->DeleteLocalRef(bitmapClass);
    ...
    memcpy(pixels, imageData.pixels.data(), imageData.pixels.size() * sizeof(uint32_t));
    AndroidBitmap_unlockPixels(env, bitmap);
    return bitmap;
}
