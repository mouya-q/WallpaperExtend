...
Image Processor::process(const Image& input, int targetW, int targetH) {
    ...
    std::memset(output.data(), 0, output.data.size() * sizeof(uint32_t));
...
void Processor::extendTop(Image& output, const Image& input, int extendH) {
    ...
    // 创建拉伸缓冲区
    std::vector<uint32_t> stretched(extendH * output.width());
    ...
    for (int y = 0; y < extendH; y++) {
        ...
        for (int x = 0; x < output.width(); x++) {
            ...
            stretched[y * output.width() + x] = input.at(srcX, srcYInt);
        }
    }
    ...
    if (blurRad > 0) {
        // 在 stretched 上应用模糊
        // 简化处理：直接在 stretched 上做水平和垂直模糊
        std::vector<uint32_t> temp = stretched;

        // 缩小处理尺寸以提升性能
        const int procW = std::min(output.width(), config.maxDimension);
        const int procH = std::min(extendH, config.maxDimension);
        const float scaleX = static_cast<float>(procW) / output.width();
        ...
        for (int y = 0; y < procH; y++) {
            for (int x = 0; x < procW; x++) {
                const int sx = std::min(static_cast<int>(x / scaleX), output.width() - 1);
                const int sy = std::min(static_cast<int>(y / scaleY), extendH - 1);
                small[y * procW + x] = stretched[sy * output.width() + sx];
            }
        }
        ...
        for (int y = 0; y < extendH; y++) {
            for (int x = 0; x < output.width(); x++) {
                ...
                stretched[y * output.width() + x] = small[sy * procW + sx];
            }
        }
    }
    ...
    for (int y = 0; y < extendH; y++) {
        for (int x = 0; x < output.width(); x++) {
            output.at(x, y) = stretched[y * output.width() + x];
        }
    }
...
void Processor::applyBlur(Image& image, int radius) {
    ...
    std::vector<uint32_t> pixels = image.data;
    stackBlurH(pixels.data(), image.width(), image.height(), radius);
    stackBlurV(pixels.data(), image.width(), image.height(), radius);
...
uint32_t Processor::sampleTopColor(const Image& image, float ratio) {
    ...
    if (count == 0) return 0xFFFFFFFF; // White
